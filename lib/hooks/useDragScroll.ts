...
const useDragScroll = <TElement extends HTMLElement>(
	...
	const dragContainerClasses = cn(
		"scrollbar-hide flex w-full cursor-grab snap-x snap-mandatory flex-row overflow-y-clip overflow-x-scroll px-4 md:px-[80px]",
		usage === "desktopOnly" && "max-md:cursor-default max-md:flex-col"
	);

	const dragItemClasses = "snap-center" as const;
