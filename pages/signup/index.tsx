...
import { callApi, checkPasswordStrength, zodValidator, type SignUpType } from '@/lib';
...
const SignUp = () => {
	...
	const onSubmit: SubmitHandler<SignUpType> = async (data: SignUpType) => {
		const { data: responseData, error } = await callApi<ApiResponse>('/auth/signup', {
			email: data.email,
		...
		>
			...
			>
				{message.message !== '' && !message.error ? (
					<p className={`rounded-md bg-abeg-error-40 p-4 text-sm font-medium text-abeg-error-20`}>{message.message}</p>
				) : (
					message.error &&
				...
				<div className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-4 lg:gap-6 sm:space-y-0">
					<div className="space-y-1">
						...
						/>
						{errors.firstName && <p className="mt-2 text-sm text-abeg-primary">{errors.firstName.message}</p>}
					</div>
					<div className="space-y-1">
						<label
							htmlFor="lastName"
						...
						/>
						{errors.lastName && <p className="mt-2 text-sm text-abeg-primary">{errors.lastName.message}</p>}
					</div>
				</div>
				...
				<div className="space-y-1">
					...
					/>
					{errors.email && <p className="mt-2 text-sm text-abeg-primary">{errors.email.message}</p>}
				</div>
					...
					<div className="space-y-1">
						...
						{password.length > 0 && (
							<div>
								...
								<p
									className={`${
										result < 2 ? 'text-text-red' : result === 2 ? 'text-yellow-500' : 'text-green-500'
									} text-sm`}
								>
						...
						{errors.password && <p className="mt-2 text-sm text-abeg-primary">{errors.password.message}</p>}
					</div>
					<div className="space-y-1">
						<label
						...
						/>
						{errors.confirmPassword && <p className="text-sm text-abeg-primary">{errors.confirmPassword.message}</p>}
					</div>
				</div>
				<div className="mt-2 flex flex-col">
					...
					{errors.terms && <p className="mt-2 text-sm text-abeg-primary">{errors.terms.message}</p>}
				</div>
				<CloudFlareTurnStile
				...
				<div className="flex flex-col items-center space-y-MSignupFormGap lg:space-y-DSignupFormGap">
					<Button
						...
						className="bg-abeg-primary py-4 mt-2 text-white"
						fullWidth
					...
					<p className="text-center text-sm">
						...
						<Link
							href="/signin"
							className="font-medium text-abeg-primary"
						>
							Login
						</Link>
					</p>
