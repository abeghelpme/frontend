...
const SingleCampaignProgress = ({
	...
	const remaining = amountRaised >= goal || goal == null ? 0 : Math.floor(goal - amountRaised);
	...
	return (
		<article className="pt-4 flex flex-col gap-2">
			...
			<div className="flex items-center justify-between">
				<p className="text-abeg-primary text-xs font-semibold">₦ {amountRaised} raised</p>

				<p className="text-abeg-primary text-xs font-semibold ">₦ {remaining} remaining</p>
			</div>
		</article>
	);
};
	...
	return (
		<article className="pt-4 flex flex-col gap-3">
			<div className="flex flex-col gap-3">
				<ProgressBar value={donationProgress} className="progress-unfilled:h-[14px]" />
				<div className="flex items-center justify-between bor">
					<p className="text-sm font-semibold">
						{donationProgress > 100 ? "100" : donationProgress}% Funded
					</p>
