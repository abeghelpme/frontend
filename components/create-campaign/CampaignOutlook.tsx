...
function CampaignOutlook(props: CampaignOutlookProps) {
	...
	return (
		<main className="flex w-full flex-col items-center pb-20 text-abeg-text">
				...
				<div className="relative mt-6">
					<TagList
						...
						render={(tag, index) => (
							<li
								key={`${tag}-${index}`}
								className="rounded-[20px] border border-white bg-white/30 px-[30px] py-1 text-[18px] font-bold "
							>
								<p>{tag}</p>
							</li>
						)}
			...
			<section className="mx-6 mt-16 space-y-5 px-[18px] py-6">
				<article>
			...
			<section className="mt-14 px-6 text-[18px]">
				<article>
					<Heading
						as="h3"
					...
					<div
						className="mt-6 min-h-16 text-justify"
			...
			<section className="mt-[72px] w-full pl-6">
				<header>
					<div className="space-y-2.5">
						<Heading as="h3" className="text-[18px] font-normal">
							Donate Today
						</Heading>
						...
						<p>Join our community of change makers and make an impact today</p>
					</div>
				...
				<CardList
					...
					each={[...Array(4).keys()]}
					render={(item) => (
						<Card key={item} as="li" className="w-full shrink-0 space-y-[25px]">
							<Card.Header
								className="h-[263px] max-w-[383px] rounded-md"
								style={{
									backgroundImage: `url(/assets/images/dashboard/dashboardImage.png)`,
								}}
							>
								<article className="flex size-full select-none flex-col items-start justify-between p-[25px] text-white">
									<figure className="flex items-center gap-1 rounded-md bg-abeg-text/30 p-2 text-[11px] backdrop-blur-md">
										<LocationIcon />
